# Amazon Marketplace Clean Markdown — V11 Card-Ready Reference, Benchmark/Scope Corrected Unified Edges
...
document_metadata:
  ...
  scope: marketplace_specific_clean_markdown_for_deterministic_parser_benchmark_scope_corrected_after_v10
  ...
  cleanup_version: v11_benchmark_scope_manifest_correction
  cleanup_focus:
  - replace lazy order_to_settlement workflow with DOCX OMS-to-settlement reconciliation steps
  - remove weak process_variant cards
  - remap return and fee transitions away from order_to_settlement
  - convert revenue_vs_cashflow constraint into a reporting-scope rule
  - encode benchmarks as structured metric-card guidance instead of open review
  - replace remaining generic workflow-step descriptions with DOCX-backed semantics
  - correct schema/type mismatches and fee-preview mandatory filters
  - replace pseudo metric implementation formulas with SQL-like deterministic formulas
  - add missing SQL pattern blocks for referenced sql_ref values
  - allow documented benchmark maps and group_level_id scope specs inside canonical cards without creating account-binding cards
```
## 0.1 Redraft Notes — Cleaned Unified Edges

This V11 correction keeps the V10 audit fixes and corrects benchmark/scope treatment. Documented benchmarks are now represented as structured `benchmarks:` fields on metric cards, and documented `group_level_id` values are allowed as scope/benchmark qualifiers without creating tenant, group, platform-account, or account-binding cards.

Edge count after correction: **942** candidate edges. Missing edge references: **0**.
...
- Allow documented `group_level_id` values to appear as `documented_scope_values` on scope-column cards and as benchmark qualifiers on metric cards; do not convert them into tenant, group, platform_account, or account_data_binding cards.
- Allow structured `benchmarks` fields on metric cards when the benchmark values are explicitly documented; treat them as guidance and investigation triggers unless a separate validation rule says otherwise.
...
- issue_id: cleanup.amazon.order_to_settlement.lazy_workflow_steps
  problem: Generic lifecycle workflow steps were created for order_to_settlement, but the DOCX describes an OMS-to-settlement reconciliation with OMS revenue, settlement product_sales, and yet-to-be-settled gap.
  resolution: Removed generic order_recorded/deductions/netted/net_posted workflow steps and replaced them with three DOCX-backed reconciliation steps.
  status: resolved
- issue_id: cleanup.amazon.return_transitions_wrong_process
  problem: Return/refund and fee-correction transitions were attached to order_to_settlement.
  resolution: Remapped return/refund transitions to return_to_settlement and fee-correction transition to expected_vs_actual_fee_validation.
  status: resolved
- issue_id: cleanup.amazon.process_variants_lazy
  problem: Fulfilment and India-vs-international return differences were modeled as process_variant cards without a material process definition.
  resolution: Removed process_variant cards; retained evidence-backed value profiles and reconciliation variants where appropriate.
  status: resolved
- issue_id: cleanup.amazon.revenue_vs_cashflow_constraint_unclear
  problem: revenue_vs_cashflow was modeled as an execution constraint set, making the meaning unclear.
  resolution: Converted it to rule.amazon.settlement_reporting_type_scope with explicit Order/Refund versus all-type settlement reporting semantics.
  status: resolved
- issue_id: cleanup.amazon.benchmark_review_unnecessary
  problem: Benchmark values were explicitly documented but left as an open benchmark_context review.
  resolution: Encoded benchmark tables as structured metric-card benchmark maps plus guidance-only rule and marked the review item resolved.
  status: resolved
- issue_id: cleanup.amazon.revenue_per_unit_unsupported_implementation
  problem: The Amazon OMS revenue_per_unit implementation referenced quantity without a created amazon_oms.quantity column card.
  resolution: Removed the Amazon-specific implementation pending schema confirmation; retained the generic metric definition.
  status: resolved
- issue_id: cleanup.amazon.remaining_lazy_workflow_steps
  problem: V9 still had generic workflow-step descriptions such as return recorded, expected fee loaded, root cause reviewed, and recovery amount recorded.
  resolution: Replaced them with DOCX-backed return cash-flow, fee recovery, fee variance, and SAFE-T data-signature semantics.
  status: resolved
- issue_id: cleanup.amazon.schema_type_mismatches
  problem: V9 assigned date/decimal types to several fields that the DOCX documents as varchar/timestamp, and treated fee_preview is_duplicated as optional.
  resolution: Corrected amazon_disbursment posted_date/settlement_date, fee_preview dimension fields, and fee_preview mandatory filters.
  status: resolved
- issue_id: cleanup.amazon.pseudo_metric_implementations
  problem: V9 contained metric implementations with placeholder formulas such as reversed fees, eligible return refund value, and principal_refunded.
  resolution: Replaced them with SQL-like deterministic formulas and explicit scope notes.
  status: resolved
- issue_id: cleanup.amazon.missing_sql_patterns
  problem: Several cards referenced sql_ref values with no matching sql_pattern block.
  resolution: Added SQL pattern blocks for every referenced sql_ref in candidate cards.
  status: resolved
- issue_id: cleanup.amazon.benchmark_scope_field_correction
  problem: V10 flattened benchmarks into benchmark_guidance strings and did not include SAFE-T benchmark guidance or documented group_level_id benchmark qualifiers in canonical metric cards.
  resolution: Added structured benchmarks maps to seller realization, return rate, and SAFE-T recovery metrics; allowed documented group_level_id specs as canonical scope/benchmark qualifiers while keeping account-binding out of scope.
  status: resolved
```

## 0.2 Manifest Additions — Benchmark and Scope Specs
...
- table_id: table.zs_observe.amazon_disbursment
  ...
  notes: Table name is misspelled in source as amazon_disbursment; match exactly. posted_date is varchar; settlement_date is timestamp.
...
candidate_card:
  ...
  fields:
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: International context is evidence-backed, but amazon_returns active analytics remain gated by the returns availability rule.
```
### 5.3 Domain Cards
...
candidate_card:
  ...
  fields:
    ...
    date_column: settlement_date; posted_date is source varchar string
    quality_notes: Table name is misspelled in source as amazon_disbursment; match exactly. posted_date is varchar, settlement_date is timestamp.
    platform_context: platform_context.amazon.in
    domain: domain.marketplace.amazon.disbursement
    evidence_refs:
    - ev.amazon_disbursment.purpose.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Table name is misspelled in source as amazon_disbursment; match exactly. posted_date is varchar, settlement_date is timestamp.
```
```yaml
candidate_card:
  card_type: table
...
candidate_card:
  ...
  fields:
    ...
    scope_guardrail: column_only_not_domain_ownership
    documented_scope_values:
      amazon_india_primary: 22
      amazon_india_secondary: 26
      amazon_international_us_fr_es: 123
    scope_value_notes: Documented source values only; use as filters/benchmark qualifiers, not account-binding cards.
    evidence_refs:
    - ev.amazon.tenant_scope_mention.001
    confidence: high
    review_status: accepted
...
candidate_card:
  ...
  fields:
    ...
    documented_scope_values:
      ...
      amazon_india_secondary: 26
      amazon_international_us_fr_es: 123
    scope_value_notes: Settlement benchmark contexts explicitly reference 22/26 for India and 123 for US/international; runtime selection remains external.
    evidence_refs:
    - ev.amazon_settlement.quality.001
    - ev.amazon.tenant_scope_mention.001
    confidence: high
...
candidate_card:
  ...
  fields:
    ...
    column_name: settlement_date
    data_type: timestamp
    business_meaning: Settlement period timestamp for disbursement row
    semantic_category: date
    scope_guardrail: marketplace_semantic_column
    evidence_refs:
    - ev.amazon_disbursment.columns.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX lists settlement_date as timestamp in amazon_disbursment.
```
```yaml
candidate_card:
  card_type: column
  ...
  fields:
    ...
    data_type: varchar
    business_meaning: Posting date string from source; do not assume native date operations without parsing
    semantic_category: date_string
    scope_guardrail: marketplace_semantic_column
    evidence_refs:
    - ev.amazon_disbursment.columns.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX lists posted_date as varchar; use settlement_date for native timestamp filtering when possible.
```
```yaml
candidate_card:
  card_type: column
...
candidate_card:
  ...
  fields:
    ...
    documented_scope_values:
      amazon_india_primary: 22
      amazon_india_secondary: 26
    scope_value_notes: Disbursement table is India-only in this DOCX; keep as scope column, not account-binding.
    evidence_refs:
    - ev.amazon.tenant_scope_mention.001
    confidence: high
    review_status: accepted
...
candidate_card:
  ...
  fields:
    ...
    data_type: varchar
    business_meaning: Longest package side in cm, stored as source string
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX lists longest_side as varchar; cast to numeric before recomputing volumetric weight.
```
```yaml
candidate_card:
  card_type: column
  ...
  fields:
    ...
    data_type: varchar
    business_meaning: Median package side in cm, stored as source string
    semantic_category: dimension
    scope_guardrail: marketplace_semantic_column
    evidence_refs:
    - ev.amazon_fee_preview.weights.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX lists median_side as varchar; cast to numeric before recomputing volumetric weight.
```
```yaml
candidate_card:
  card_type: column
  ...
  fields:
    ...
    data_type: varchar
    business_meaning: Shortest package side in cm, stored as source string
    semantic_category: dimension
    scope_guardrail: marketplace_semantic_column
    evidence_refs:
    - ev.amazon_fee_preview.weights.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX lists shortest_side as varchar; cast to numeric before recomputing volumetric weight.
```
```yaml
candidate_card:
  card_type: column
...
candidate_card:
  ...
  fields:
    ...
    scope_guardrail: column_only_not_domain_ownership
    documented_scope_values:
      amazon_international_us_canada: 123
    scope_value_notes: amazon_returns is documented as international returns with group_level_id 123 and currently inactive.
    evidence_refs:
    - ev.amazon_returns.columns.001
    - ev.amazon.tenant_scope_mention.001
    confidence: high
...
candidate_card:
  ...
  fields:
    ...
    - einvoicecancel
    - 'null'
    value_semantics: forward=positive shipped sale; reverse=negative customer return; cancel=zero pre-ship cancellation; replacement=zero replacement; einvoicecancel=zero
      GST e-invoice cancellation; null=international rows
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: forward=positive shipped sale; reverse=negative customer return; cancel=zero pre-ship cancellation; replacement=zero replacement; einvoicecancel=zero
      GST e-invoice cancellation; null=international rows
```
```yaml
candidate_card:
...
candidate_card:
  ...
  fields:
    ...
    benchmarks:
      ...
      source: docx section 2 (Settlement & Realization Metrics) and amazon_settlement Seller Realization Rate section
    candidate_notes: Percentage of gross product sales received as settlement after deductions. Benchmarks are documented guidance, not hard pass/fail thresholds.
```
```yaml
candidate_card:
  card_type: metric
...
candidate_card:
  ...
  fields:
    ...
    marketplace_only: true
    benchmarks:
      india_general_healthy: '<15 percent'
      india_general_concerning: '>25 percent'
      apparel_healthy: '<20 percent'
      apparel_concerning: '>35 percent'
      electronics_healthy: '<10 percent'
      electronics_concerning: '>20 percent'
      high_value_items_over_2000_healthy: '<12 percent'
      high_value_items_over_2000_concerning: '>20 percent'
      source: docx section 8 (Return Rate Health Benchmarks)
    candidate_notes: Returned orders divided by forward orders. Return-rate benchmarks are guidance bands, not universal validation thresholds.
```
```yaml
candidate_card:
  card_type: metric
...
candidate_card:
  ...
  fields:
    ...
    business_definition: Percentage of eligible-return refund value recovered via SAFE-T reimbursement credits.
    colloquial_names:
    - SAFE-T recovery rate
    - SAFE-T recovery
    - safe-t recovery
    - reimbursement recovery rate
    metric_pattern: safe_t_credits_over_eligible_refund_value
    default_grain: settlement_period
    domain: domain.marketplace.amazon.reimbursements_claims; domain.marketplace.amazon.returns
    scope: generic marketplace metric implemented by Amazon-specific metric_implementation cards
    benchmarks:
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Recovery on eligible Amazon-caused or buyer-fraud returns; tied to SAFE-T eligibility codes in amazon_returns and SAFE-T Reimbursement type in settlement/disbursement.
```
```yaml
candidate_card:
  card_type: metric
...
candidate_card:
  ...
  fields:
    ...
    platform_context: platform_context.amazon.in
    source_tables: table.zs_observe.amazon_disbursment
    formula: ABS(SUM(CASE WHEN mp_fee_type = 'ItemFees' AND charged_amount_type = 'commission' THEN charged_amount ELSE 0 END)) / NULLIF(SUM(CASE WHEN mp_fee_type
...
candidate_card:
  ...
  fields:
    ...
    platform_context: platform_context.amazon.in
    source_tables: table.zs_observe.amazon_disbursment
    formula: ABS(SUM(CASE WHEN mp_fee_type = 'ItemFees' THEN charged_amount ELSE 0 END)) / NULLIF(SUM(CASE WHEN mp_fee_type = 'ItemPrice' AND charged_amount_type
...
candidate_card:
  ...
  fields:
    ...
    platform_context: platform_context.amazon.in
    source_tables: table.zs_observe.amazon_disbursment
    formula: ABS(SUM(CASE WHEN mp_fee_type = 'ItemFees' AND charged_amount_type IN ('shipping fee', 'shipping chargeback') THEN charged_amount ELSE 0 END)) / NULLIF(SUM(CASE
      WHEN mp_fee_type = 'ItemPrice' AND charged_amount_type = 'principal' THEN charged_amount ELSE 0 END), 0)
    required_filters: is_active = true AND transaction_type = 'Order'
    grain: sku
...
candidate_card:
  ...
  fields:
    ...
    required_filters: is_active = true AND zen_status = true AND is_duplicated = false
    grain: sku
...
candidate_card:
  ...
  fields:
    ...
    platform_context: platform_context.amazon.in
    source_tables: table.zs_observe.amazon_disbursment
    formula: ABS(SUM(CASE WHEN mp_fee_type = 'ItemFees' AND charged_amount_type = 'commission' THEN charged_amount ELSE 0 END))
    required_filters: is_active = true AND transaction_type = 'Order'
    grain: order_item
    sql_ref: sql.amazon.disbursement.forward_fee
    metric_pattern: absolute_commission_forward_filter
    notes: Forward fee in the DOCX is commission on forward/order transactions, not all ItemFees.
    evidence_refs:
    - ev.amazon.kpi.fee_cost.001
    - ev.amazon_disbursment.taxonomy.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Forward fee in the DOCX is commission on forward/order transactions, not all ItemFees.
```
```yaml
candidate_card:
  card_type: metric_implementation
...
candidate_card:
  ...
  fields:
    ...
    platform_context: platform_context.amazon.in
    source_tables: table.zs_observe.amazon_disbursment
    formula: (SUM(CASE WHEN transaction_type = 'Refund' THEN charged_amount ELSE 0 END) + SUM(CASE WHEN transaction_type = 'Fulfillment Fee Refund' THEN charged_amount
      ELSE 0 END)) / NULLIF(ABS(SUM(CASE WHEN transaction_type = 'Order' THEN charged_amount ELSE 0 END)), 0)
    required_filters: is_active = true AND mp_fee_type IN ('ItemFees', 'Item Fee Adjustment', 'ItemTCS', 'ItemTDS')
    grain: fee_type
    sql_ref: sql.amazon.disbursement.fee_recovery_on_returns
    metric_pattern: reversed_fees_over_charged_fees
    notes: Compute at charged_amount_type grain; numerator is actual refund/fee-refund recoveries, denominator is original Order charge magnitude.
    evidence_refs:
    - ev.amazon.kpi.settlement.001
    - ev.amazon_disbursment.transaction_types.001
    - ev.amazon_disbursment.queries.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Compute at charged_amount_type grain; numerator is actual refund/fee-refund recoveries, denominator is original Order charge magnitude.
```
```yaml
candidate_card:
  card_type: metric_implementation
...
candidate_card:
  ...
  fields:
    ...
    platform_context: platform_context.amazon.in
    source_tables: table.zs_observe.amazon_oms
    formula: COUNT(DISTINCT order_id) FILTER (WHERE transaction_type = 'cancel') / NULLIF(COUNT(DISTINCT order_id) FILTER (WHERE transaction_type IN ('forward', 'cancel')),
      0)
    required_filters: is_active = true
    grain: sku
...
candidate_card:
  ...
  fields:
    ...
    platform_context: platform_context.amazon.in
    source_tables: table.zs_observe.amazon_oms
    formula: SUM(CASE WHEN transaction_type IN ('forward', 'reverse') THEN charged_amount ELSE 0 END) / NULLIF(COUNT(DISTINCT CASE WHEN transaction_type='forward'
      THEN order_id END), 0)
    required_filters: is_active = true
    grain: order
...
candidate_card:
  ...
  fields:
    ...
    formula: SUM(CASE WHEN s.type = 'SAFE-T Reimbursement' THEN s.total ELSE 0 END) / NULLIF(SUM(CASE WHEN r.return_reason IN ('DAMAGED_BY_FC','DAMAGED_BY_CARRIER','SWITCHEROO','UNAUTHORIZED_PURCHASE','NOT_AS_DESCRIBED')
      THEN ABS(r.charged_amount) ELSE 0 END), 0)
    required_filters: s.is_active = true; r.is_active = true; amazon_returns must be active/re-ingested; NOT_AS_DESCRIBED eligibility may require listing-accuracy
      review
    grain: settlement_period
    sql_ref: sql.amazon.settlement.reimbursements_safet
    metric_pattern: safe_t_credits_over_eligible_refund_value
    notes: Period-level deterministic proxy for eligible return refund value using documented reason codes; not a hard claim-eligibility decision.
    evidence_refs:
    - ev.amazon.safet.001
    - ev.amazon_settlement.kpis.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Period-level deterministic proxy for eligible return refund value using documented reason codes; not a hard claim-eligibility decision.
```
```yaml
candidate_card:
  card_type: metric_implementation
  ...
  fields:
    ...
    platform_context: platform_context.amazon.in
    source_tables: table.zs_observe.amazon_disbursment
    formula: ABS(SUM(CASE WHEN transaction_type = 'Refund' AND mp_fee_type = 'ItemPrice' AND charged_amount_type = 'principal' THEN charged_amount ELSE 0 END)) +
      GREATEST(0, ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type = 'shipping fee' THEN charged_amount ELSE 0 END)) - SUM(CASE WHEN transaction_type
      = 'Fulfillment Fee Refund' AND charged_amount_type = 'shipping fee' THEN charged_amount ELSE 0 END)) + GREATEST(0, ABS(SUM(CASE WHEN transaction_type = 'Order'
      AND charged_amount_type = 'tech fee' THEN charged_amount ELSE 0 END)) - SUM(CASE WHEN transaction_type = 'Fulfillment Fee Refund' AND charged_amount_type =
      'tech fee' THEN charged_amount ELSE 0 END)) + ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type = 'pickup fee' THEN charged_amount ELSE 0
      END)) + GREATEST(0, ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type = 'commission' THEN charged_amount ELSE 0 END)) - SUM(CASE WHEN transaction_type
      = 'Refund' AND charged_amount_type = 'refund commission' THEN charged_amount ELSE 0 END)) + GREATEST(0, ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type
      = 'closing fee' THEN charged_amount ELSE 0 END)) - SUM(CASE WHEN transaction_type = 'Refund' AND charged_amount_type = 'closing fee' THEN charged_amount ELSE
      0 END)) + 0.60 * ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type = 'tcs gst' THEN charged_amount ELSE 0 END))
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: SQL-like implementation of the DOCX true-return-cost framework; uses actual fee recovery rows when available and the documented 60% TCS timing-loss
      approximation.
```
```yaml
candidate_card:
...
candidate_card:
  ...
  fields:
    ...
    formula: fp.gross_commission + SUM(CASE WHEN d.mp_fee_type = 'ItemFees' AND d.transaction_type = 'Order' THEN d.charged_amount ELSE 0 END)
    required_filters: fp.is_active = true AND fp.zen_status = true AND fp.is_duplicated = false; d.is_active = true; aggregate d by sku_id before join
    grain: sku
    sql_ref: sql.amazon.fee_preview.expected_vs_actual
    metric_pattern: expected_fee_plus_negative_actual_fee
    notes: Actual ItemFees are negative, so expected + actual yields variance; disbursement must be aggregated to SKU before joining.
    evidence_refs:
    - ev.amazon.recon.fee_preview_actual.001
    - ev.amazon_fee_preview.recon.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Actual ItemFees are negative, so expected + actual yields variance; disbursement must be aggregated to SKU before joining.
```
...
candidate_card:
  ...
  fields:
    ...
    formula_expression: charged_amount = item_amount_excluding_tax + total_tax + shipping_amount + gift_wrap_amount - item_promo_discount - shipping_promo_discount
...
candidate_card:
  ...
  fields:
    ...
    formula_expression: total ≈ product_sales + selling_fees + fba_fees + shipping_credits + promotional_rebates + gift_wrap_credits + other_transaction_fees + other
      - tcs_igst - tcs_cgst - tcs_sgst - tds
    platform_context: platform_context.amazon.in
...
candidate_card:
  ...
  fields:
    ...
    formula_expression: principal_refunded + shipping_fee_non_recoverable + tech_fee_non_recoverable + pick_pack_fee_non_recoverable + approx 50% commission + approx
      58% closing_fee + approx 60% TCS timing loss
    platform_context: platform_context.amazon.in
...
candidate_card:
  ...
  name: Amazon OMS to settlement reconciliation framework
  fields:
    process_key: order_to_settlement_reconciliation
    platform_context: platform_context.amazon.in
    process_description: Reconciles OMS forward-order gross sales against Amazon settlement Order product_sales and classifies the remaining gap as yet-to-be-settled
      timing difference.
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: 'Cleaned from generic order lifecycle to the DOCX-backed OMS ↔ Settlement reconciliation: OMS forward revenue, settlement product_sales Orders,
      and yet-to-be-settled gap.'
```
```yaml
candidate_card:
  card_type: business_process
...
candidate_card:
  card_type: workflow_step
  card_id: workflow_step.amazon.order_to_settlement.oms_forward_revenue_source_of_truth
  name: 'Amazon OMS to settlement reconciliation: OMS forward revenue source of truth'
  fields:
    process: business_process.amazon.order_to_settlement
    step_key: oms_forward_revenue_source_of_truth
    step_order: '1'
    step_description: OMS Revenue on forward orders is the gross-sales source of truth for the reconciliation.
    evidence_refs:
    - ev.amazon.recon.oms_settlement.001
    confidence: high
    review_status: accepted
    create_action: create
    marketplace_only: true
    candidate_notes: 'Replaces lazy order_recorded lifecycle step with DOCX reconciliation side: OMS Revenue (forward orders) = Gross Sales source of truth.'
```
```yaml
candidate_card:
  card_type: workflow_step
  card_id: workflow_step.amazon.order_to_settlement.settlement_product_sales_order_view
  name: 'Amazon OMS to settlement reconciliation: settlement product_sales order view'
  fields:
    process: business_process.amazon.order_to_settlement
    step_key: settlement_product_sales_order_view
    step_order: '2'
    step_description: Settlement product_sales on Order rows is Amazon's version of those sales.
    evidence_refs:
    - ev.amazon.recon.oms_settlement.001
    confidence: high
    review_status: accepted
    create_action: create
    marketplace_only: true
    candidate_notes: 'Replaces lazy deductions/settlement lifecycle steps with DOCX reconciliation side: Settlement product_sales (Orders) = Amazon''s version of
      those sales.'
```
```yaml
candidate_card:
  card_type: workflow_step
  card_id: workflow_step.amazon.order_to_settlement.yet_to_be_settled_gap
  name: 'Amazon OMS to settlement reconciliation: yet-to-be-settled gap'
  fields:
    process: business_process.amazon.order_to_settlement
    step_key: yet_to_be_settled_gap
    step_order: '3'
    step_description: The reconciliation gap is the yet-to-be-settled timing difference for orders in the billing cycle.
    evidence_refs:
    - ev.amazon.recon.oms_settlement.001
    - ev.amazon.settlement_cycle.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX-backed gap semantics; this is a timing gap, not a material settlement discrepancy by default.
```
...
candidate_card:
  ...
  fields:
    ...
    step_key: return_reverse_event_captured
    step_order: '1'
    step_description: OMS reverse rows or return records identify the customer return/refund event that reduces revenue.
    evidence_refs:
    - ev.amazon.return_analysis.001
    - ev.amazon.settlement_cycle.001
    - ev.amazon_disbursment.transaction_types.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: 'DOCX-backed return entry point: India returns use amazon_oms transaction_type = reverse; international return reason data uses amazon_returns
      when active.'
```
```yaml
candidate_card:
  ...
  fields:
    process: business_process.amazon.return_to_settlement
    step_key: refund_debited_in_settlement
    step_order: '2'
    step_description: When a return occurs after the original settlement period closes, the refund is debited in a later settlement period.
    evidence_refs:
    - ev.amazon.return_analysis.001
    - ev.amazon.settlement_cycle.001
    - ev.amazon_disbursment.transaction_types.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: 'DOCX settlement-cycle behavior: original forward order may already be paid; refund appears as later debit.'
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    process: business_process.amazon.return_to_settlement
    step_key: fee_recovery_assessed_by_fee_type
    step_order: '3'
    step_description: Refund and Fulfillment Fee Refund rows are evaluated by fee type to determine commission, closing-fee, shipping/tech, TCS, and TDS recovery.
    evidence_refs:
    - ev.amazon.return_analysis.001
    - ev.amazon.settlement_cycle.001
    - ev.amazon_disbursment.transaction_types.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: 'DOCX fee recovery matrix: commission/closing/TCS partial, shipping/tech via fulfillment fee refund only, pickup/TDS not directly recovered.'
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    process: business_process.amazon.return_to_settlement
    step_key: non_recovered_return_cost_recorded
    step_order: '4'
    step_description: Non-recovered shipping, tech, pick-and-pack, commission, closing-fee, and TCS timing components are rolled into true return loss.
    evidence_refs:
    - ev.amazon.return_analysis.001
    - ev.amazon.settlement_cycle.001
    - ev.amazon_disbursment.transaction_types.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX true return cost formula, not a generic lifecycle step.
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    ...
    step_key: expected_fee_preview_loaded
    step_order: '1'
    step_description: Expected fee is taken from amazon_fee_preview.gross_commission and component fields at SKU grain.
    evidence_refs:
    - ev.amazon.recon.fee_preview_actual.001
    - ev.amazon_fee_preview.recon.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX Fee Preview ↔ Actual Fee reconciliation expected side.
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    process: business_process.amazon.expected_vs_actual_fee_validation
    step_key: actual_itemfees_aggregated_to_sku
    step_order: '2'
    step_description: Actual Amazon ItemFees from amazon_disbursment Order rows are aggregated to SKU before comparison.
    evidence_refs:
    - ev.amazon.recon.fee_preview_actual.001
    - ev.amazon_fee_preview.recon.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX requires aggregate-before-join because disbursement is line-level while fee preview is SKU-level.
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    process: business_process.amazon.expected_vs_actual_fee_validation
    step_key: expected_plus_negative_actual_variance_computed
    step_order: '3'
    step_description: Fee variance is computed as expected gross_commission plus actual ItemFees because actual fee charges are negative.
    evidence_refs:
    - ev.amazon.recon.fee_preview_actual.001
    - ev.amazon_fee_preview.recon.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX states actual ItemFees are negative, so expected + actual gives the variance.
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    process: business_process.amazon.expected_vs_actual_fee_validation
    step_key: variance_root_cause_classified
    step_order: '4'
    step_description: Variance is classified into price difference, weight/dimension mismatch, category reclassification, or fee-tier change.
    evidence_refs:
    - ev.amazon.recon.fee_preview_actual.001
    - ev.amazon_fee_preview.recon.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX lists these variance root causes explicitly.
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    ...
    step_key: safe_t_eligible_return_signal_identified
    step_order: '1'
    step_description: SAFE-T eligibility signals are damage-by-FC/carrier, switcheroo, unauthorized/fraud indicators, or not-as-described where seller listing was
      accurate.
    evidence_refs:
    - ev.amazon.safet.001
    - ev.amazon.return_analysis.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX SAFE-T triggers and return reason taxonomy.
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    process: business_process.amazon.safe_t_reimbursement
    step_key: safe_t_credit_detected_in_settlement_or_disbursement
    step_order: '2'
    step_description: SAFE-T credit is detected as settlement type SAFE-T Reimbursement or disbursement reimbursement under Other Transactions / SAFE-T Reimbursement.
    evidence_refs:
    - ev.amazon.safet.001
    - ev.amazon.return_analysis.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX data signatures for SAFE-T in settlement and disbursement.
```
```yaml
candidate_card:
  card_type: workflow_step
  ...
  fields:
    process: business_process.amazon.safe_t_reimbursement
    step_key: safe_t_recovery_amount_and_rate_recorded
    step_order: '3'
    step_description: SAFE-T recovered amount is summed and can be compared with eligible return refund value as a recovery-rate metric.
    evidence_refs:
    - ev.amazon.safet.001
    - ev.amazon.return_analysis.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: DOCX SAFE-T recovery-rate formula and benchmark guidance.
```
...
candidate_card:
  ...
  fields:
    ...
    from_state: forward
    to_state: reverse
    transition_meaning: A shipped sale can later become a customer return represented by reverse transaction type; this belongs to return-to-settlement analysis.
    evidence_refs:
    - ev.amazon_oms.transaction_types.001
    confidence: high
    review_status: accepted
    create_action: create
    marketplace_only: true
    candidate_notes: Remapped to return_to_settlement.
```
```yaml
candidate_card:
  card_type: state_transition
  ...
  fields:
    ...
    from_state: Order
    to_state: Refund
    transition_meaning: A settlement Order row can later be offset by a Refund row; this belongs to return-to-settlement analysis.
    evidence_refs:
    - ev.amazon_settlement.columns.001
    confidence: high
    review_status: accepted
...
candidate_card:
  ...
  name: 'Amazon disbursement transition: Order fee charge to Refund fee reversal'
  fields:
    state_column: column.zs_observe.amazon_disbursment.transaction_type
    from_state: Order
    to_state: Refund
    transition_meaning: Disbursement Order fee charges can later be partially reversed on Refund transactions; this belongs to the return-to-settlement fee recovery
      process, not OMS-to-settlement reconciliation.
    evidence_refs:
    - ev.amazon_disbursment.transaction_types.001
    confidence: high
    review_status: accepted
    create_action: create
    marketplace_only: true
    candidate_notes: Remapped to return_to_settlement; not part of OMS forward revenue versus settlement Order product_sales reconciliation.
```
```yaml
candidate_card:
  card_type: state_transition
  ...
  fields:
    ...
    transition_meaning: An Amazon fee charge can later be corrected by a Fulfillment Fee Refund; this belongs to expected-vs-actual fee validation, not OMS-to-settlement
      reconciliation.
    evidence_refs:
    - ev.amazon_disbursment.transaction_types.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Remapped to expected_vs_actual_fee_validation.
```
...
candidate_card:
  ...
  fields:
    profile: reconciliation_profile.amazon.oms_to_settlement
    join_keys: order_id
    matching_rule: Compare count distinct order_id and sum OMS charged_amount against settlement product_sales for type = Order; identify OMS orders not yet in settlement
      as timing gaps.
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Compare count distinct order_id and sum OMS charged_amount against settlement product_sales for type = Order; identify OMS orders not yet in
      settlement as timing gaps.
```
```yaml
candidate_card:
  card_type: matching_logic
...
candidate_card:
  ...
  fields:
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Group/context differences affect marketplace labels and realization benchmarks; not account-binding. Accepted after cleanup as evidence-backed
      variant metadata, not lazy process population.
```
```yaml
candidate_card:
  card_type: reconciliation_variant
  ...
  fields:
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Use when component fields exist and the variance needs referral, fixed, shipping, or pick-and-pack breakdown. Accepted after cleanup as evidence-backed
      variant metadata, not lazy process population.
```
```yaml
candidate_card:
  ...
  fields:
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: India return tracking uses OMS reverse, while international return reasons use amazon_returns when active. Accepted after cleanup as evidence-backed
      variant metadata, not lazy process population.
```
...
candidate_card:
  ...
  fields:
    ...
    query_logic: 'Use OMS forward/reverse net revenue plus disbursement return-loss components: refunded principal and non-recovered shipping, tech, pickup, commission,
      closing-fee, and TCS timing loss.'
    sql_ref: sql.amazon.disbursement.return_loss
    scope_policy: runtime filters may be applied externally; do not hardcode group_level_id in canonical pattern
    evidence_refs:
    - ev.amazon.analytical_patterns.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: 'Use OMS forward/reverse net revenue plus disbursement return-loss components: refunded principal and non-recovered shipping, tech, pickup, commission,
      closing-fee, and TCS timing loss.'
```
```yaml
candidate_card:
...
candidate_card:
  ...
  fields:
    ...
    rule_text: Use is_active = true, zen_status = true, and is_duplicated = false; zen_status/is_duplicated are booleans here.
    rule_type: business_semantic_rule
    evidence_refs:
    - ev.amazon_fee_preview.quality.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Use is_active = true, zen_status = true, and is_duplicated = false; zen_status/is_duplicated are booleans here.
```
```yaml
candidate_card:
  card_type: rule
...
candidate_card:
  ...
  fields:
    ...
    rule_text: Do not hardcode group_level_id as runtime scope in generated SQL unless supplied by user/context; documented group_level_id values may be stored as scope specs and benchmark qualifiers in canonical cards.
    rule_type: business_semantic_rule
    evidence_refs:
    - ev.amazon.tenant_scope_mention.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Documented group_level_id specs are allowed as canonical scope context; runtime account/scope binding remains external.
```
```yaml
candidate_card:
  card_type: rule
...
candidate_card:
  card_type: rule
  card_id: rule.amazon.settlement_reporting_type_scope
  name: 'Settlement reporting type scope: revenue versus cash flow'
  fields:
    related_canonical_id: column.zs_observe.amazon_settlement.type
    rule_text: For settlement revenue reporting, restrict to type IN ('Order','Refund'); for settlement cash-flow/payout reporting, include all settlement type values
      because Service Fee, FBA Inventory Fee, Adjustment, SAFE-T, Reimbursements, Clawbacks, Others, and NULL rows affect payout.
    rule_type: business_semantic_rule
    evidence_refs:
    - ev.amazon_settlement.columns.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Converted from the previous revenue-vs-cashflow execution constraint because this is a reporting-scope rule, not a standalone execution constraint.
```
```yaml
candidate_card:
  card_type: rule
  card_id: rule.amazon.benchmark_guidance_only
  name: Use Amazon benchmark tables as guidance only
  fields:
    related_canonical_id:
    - metric.marketplace.seller_realization_rate
    - metric.marketplace.return_rate
    - metric.marketplace.safe_t_recovery_rate
    rule_text: Documented benchmark ranges should be surfaced as structured metric-card benchmarks and contextual guidance/investigation triggers; do not encode them as hard validation failures without
      marketplace, account, category, eligibility, and period context.
    rule_type: business_semantic_rule
    evidence_refs:
    - ev.amazon.kpi.settlement.001
    - ev.amazon.return_analysis.001
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: 'Resolves review.amazon.benchmarks.001: benchmarks are explicitly documented and should be encoded as structured guidance fields, not left as an open review item.'
```
...
candidate_card:
  ...
  fields:
    ...
    constraint_text: Do not hardcode group_level_id as runtime scope in generated SQL unless supplied by user/context; documented group_level_id values may appear as canonical scope specs or benchmark qualifiers.
    constraint_scope: marketplace_semantic_execution
    evidence_refs:
    - ev.amazon.tenant_scope_mention.001
    confidence: high
    ...
    create_action: create
    marketplace_only: true
    candidate_notes: Canonical cards may document group_level_id values; runtime or separate scope layer supplies the selected scope.
```
...
edge_coverage_summary:
  candidate_edge_count: 942
  canonical_cognee_edge_count: 871
  parser_helper_edge_count: 71
  edges_with_legacy_aliases: 451
  materialized_inverse_edges: 450
...
```yaml
candidate_edge:
  edge_id: edge.amazon.business_process.amazon.order_to_settlement.HAS_WORKFLOW_STEP.workflow_step.amazon.order_to_settlement.oms_forward_revenue_source_of_truth
  edge_type: HAS_WORKFLOW_STEP
  canonical_edge_type: HAS_WORKFLOW_STEP
  source_id: business_process.amazon.order_to_settlement
  source_type: business_process
  target_id: workflow_step.amazon.order_to_settlement.oms_forward_revenue_source_of_truth
  target_type: workflow_step
  legacy_edge_aliases: []
  inverse_edge_type: BELONGS_TO_PROCESS
  materialize_inverse: true
...
```yaml
candidate_edge:
  edge_id: edge.amazon.workflow_step.amazon.order_to_settlement.oms_forward_revenue_source_of_truth.BELONGS_TO_PROCESS.business_process.amazon.order_to_settlement
  edge_type: BELONGS_TO_PROCESS
  canonical_edge_type: BELONGS_TO_PROCESS
  source_id: workflow_step.amazon.order_to_settlement.oms_forward_revenue_source_of_truth
  source_type: workflow_step
...
```yaml
candidate_edge:
  edge_id: edge.amazon.business_process.amazon.order_to_settlement.HAS_WORKFLOW_STEP.workflow_step.amazon.order_to_settlement.settlement_product_sales_order_view
  edge_type: HAS_WORKFLOW_STEP
  canonical_edge_type: HAS_WORKFLOW_STEP
  source_id: business_process.amazon.order_to_settlement
  source_type: business_process
  target_id: workflow_step.amazon.order_to_settlement.settlement_product_sales_order_view
  target_type: workflow_step
  legacy_edge_aliases: []
  inverse_edge_type: BELONGS_TO_PROCESS
  materialize_inverse: true
...
```yaml
candidate_edge:
  edge_id: edge.amazon.workflow_step.amazon.order_to_settlement.settlement_product_sales_order_view.BELONGS_TO_PROCESS.business_process.amazon.order_to_settlement
  edge_type: BELONGS_TO_PROCESS
  canonical_edge_type: BELONGS_TO_PROCESS
  source_id: workflow_step.amazon.order_to_settlement.settlement_product_sales_order_view
  source_type: workflow_step
  target_id: business_process.amazon.order_to_settlement
  target_type: business_process
  legacy_edge_aliases:
...
```yaml
candidate_edge:
  edge_id: edge.amazon.business_process.amazon.order_to_settlement.HAS_WORKFLOW_STEP.workflow_step.amazon.order_to_settlement.yet_to_be_settled_gap
  edge_type: HAS_WORKFLOW_STEP
  canonical_edge_type: HAS_WORKFLOW_STEP
  source_id: business_process.amazon.order_to_settlement
  source_type: business_process
  target_id: workflow_step.amazon.order_to_settlement.yet_to_be_settled_gap
  target_type: workflow_step
  legacy_edge_aliases: []
  inverse_edge_type: BELONGS_TO_PROCESS
  materialize_inverse: true
...
```yaml
candidate_edge:
  edge_id: edge.amazon.workflow_step.amazon.order_to_settlement.yet_to_be_settled_gap.BELONGS_TO_PROCESS.business_process.amazon.order_to_settlement
  edge_type: BELONGS_TO_PROCESS
  canonical_edge_type: BELONGS_TO_PROCESS
  source_id: workflow_step.amazon.order_to_settlement.yet_to_be_settled_gap
  source_type: workflow_step
  target_id: business_process.amazon.order_to_settlement
  target_type: business_process
  legacy_edge_aliases:
...
```yaml
candidate_edge:
  edge_id: edge.amazon.business_process.amazon.return_to_settlement.HAS_STATE_TRANSITION.state_transition.amazon.oms_forward_to_reverse
  edge_type: HAS_STATE_TRANSITION
...
```yaml
candidate_edge:
  edge_id: edge.amazon.state_transition.amazon.oms_forward_to_reverse.BELONGS_TO_PROCESS.business_process.amazon.return_to_settlement
  edge_type: BELONGS_TO_PROCESS
  canonical_edge_type: BELONGS_TO_PROCESS
...
```yaml
candidate_edge:
  edge_id: edge.amazon.business_process.amazon.return_to_settlement.HAS_STATE_TRANSITION.state_transition.amazon.settlement_order_to_refund
  edge_type: HAS_STATE_TRANSITION
  canonical_edge_type: HAS_STATE_TRANSITION
  source_id: business_process.amazon.return_to_settlement
  source_type: business_process
...
```yaml
candidate_edge:
  edge_id: edge.amazon.state_transition.amazon.settlement_order_to_refund.BELONGS_TO_PROCESS.business_process.amazon.return_to_settlement
  edge_type: BELONGS_TO_PROCESS
  canonical_edge_type: BELONGS_TO_PROCESS
...
```yaml
candidate_edge:
  edge_id: edge.amazon.business_process.amazon.return_to_settlement.HAS_STATE_TRANSITION.state_transition.amazon.disbursment_order_fee_to_refund_fee
  edge_type: HAS_STATE_TRANSITION
  canonical_edge_type: HAS_STATE_TRANSITION
  source_id: business_process.amazon.return_to_settlement
  source_type: business_process
...
```yaml
candidate_edge:
  edge_id: edge.amazon.state_transition.amazon.disbursment_order_fee_to_refund_fee.BELONGS_TO_PROCESS.business_process.amazon.return_to_settlement
  edge_type: BELONGS_TO_PROCESS
  canonical_edge_type: BELONGS_TO_PROCESS
...
```yaml
candidate_edge:
  edge_id: edge.amazon.business_process.amazon.expected_vs_actual_fee_validation.HAS_STATE_TRANSITION.state_transition.amazon.fee_charge_to_fulfillment_fee_refund
  edge_type: HAS_STATE_TRANSITION
  canonical_edge_type: HAS_STATE_TRANSITION
...
```yaml
candidate_edge:
  edge_id: edge.amazon.state_transition.amazon.fee_charge_to_fulfillment_fee_refund.BELONGS_TO_PROCESS.business_process.amazon.expected_vs_actual_fee_validation
  edge_type: BELONGS_TO_PROCESS
  canonical_edge_type: BELONGS_TO_PROCESS
...
```yaml
candidate_edge:
  edge_id: edge.amazon.rule.amazon.settlement_reporting_type_scope.TARGETS_CARD.column.zs_observe.amazon_settlement.type
  edge_type: TARGETS_CARD
  canonical_edge_type: TARGETS_CARD
  source_id: rule.amazon.settlement_reporting_type_scope
  source_type: rule
...
```yaml
candidate_edge:
  edge_id: edge.amazon.rule.amazon.benchmark_guidance_only.TARGETS_CARD.metric.marketplace.seller_realization_rate
  edge_type: TARGETS_CARD
  canonical_edge_type: TARGETS_CARD
  source_id: rule.amazon.benchmark_guidance_only
  source_type: rule
...
```yaml
candidate_edge:
  edge_id: edge.amazon.rule.amazon.benchmark_guidance_only.TARGETS_CARD.metric.marketplace.return_rate
  edge_type: TARGETS_CARD
  canonical_edge_type: TARGETS_CARD
  source_id: rule.amazon.benchmark_guidance_only
  source_type: rule
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.actual_fee_variance.USES_TABLE.table.zs_observe.amazon_disbursment
  edge_type: USES_TABLE
  canonical_edge_type: USES_TABLE
  source_id: metric_implementation.amazon.amazon_fee_preview.actual_fee_variance
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.actual_fee_variance.USES_COLUMN.column.zs_observe.amazon_disbursment.charged_amount
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_fee_preview.actual_fee_variance
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.actual_fee_variance.USES_COLUMN.column.zs_observe.amazon_disbursment.mp_fee_type
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_fee_preview.actual_fee_variance
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.actual_fee_variance.USES_COLUMN.column.zs_observe.amazon_disbursment.transaction_type
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_fee_preview.actual_fee_variance
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.actual_fee_variance.USES_COLUMN.column.zs_observe.amazon_disbursment.sku_id
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_fee_preview.actual_fee_variance
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.actual_fee_variance.USES_COLUMN.column.zs_observe.amazon_disbursment.is_active
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_fee_preview.actual_fee_variance
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.actual_fee_variance.USES_COLUMN.column.zs_observe.amazon_fee_preview.is_duplicated
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_fee_preview.actual_fee_variance
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_disbursment.forward_fee.USES_COLUMN.column.zs_observe.amazon_disbursment.charged_amount_type
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_disbursment.forward_fee
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_disbursment.fee_recovery_rate.USES_COLUMN.column.zs_observe.amazon_disbursment.charged_amount
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_disbursment.fee_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_disbursment.fee_recovery_rate.USES_COLUMN.column.zs_observe.amazon_disbursment.charged_amount_type
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_disbursment.fee_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_disbursment.fee_recovery_rate.USES_COLUMN.column.zs_observe.amazon_disbursment.mp_fee_type
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_disbursment.fee_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_disbursment.return_loss.USES_COLUMN.column.zs_observe.amazon_disbursment.charged_amount
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_disbursment.return_loss
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_disbursment.return_loss.USES_COLUMN.column.zs_observe.amazon_disbursment.charged_amount_type
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_disbursment.return_loss
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_disbursment.return_loss.USES_COLUMN.column.zs_observe.amazon_disbursment.mp_fee_type
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_disbursment.return_loss
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_disbursment.return_loss.USES_COLUMN.column.zs_observe.amazon_disbursment.order_id
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_disbursment.return_loss
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate.USES_TABLE.table.zs_observe.amazon_returns
  edge_type: USES_TABLE
  canonical_edge_type: USES_TABLE
  source_id: metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate.USES_COLUMN.column.zs_observe.amazon_returns.order_id
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate.USES_COLUMN.column.zs_observe.amazon_returns.return_reason
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate.USES_COLUMN.column.zs_observe.amazon_returns.charged_amount
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate.USES_COLUMN.column.zs_observe.amazon_returns.is_active
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate.USES_COLUMN.column.zs_observe.amazon_settlement.order_id
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_settlement.safe_t_recovery_rate
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.fee_burden.USES_COLUMN.column.zs_observe.amazon_fee_preview.is_duplicated
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_fee_preview.fee_burden
  source_type: metric_implementation
...
```yaml
candidate_edge:
  edge_id: edge.amazon.metric_implementation.amazon.amazon_fee_preview.volumetric_excess_percent.USES_COLUMN.column.zs_observe.amazon_fee_preview.is_duplicated
  edge_type: USES_COLUMN
  canonical_edge_type: USES_COLUMN
  source_id: metric_implementation.amazon.amazon_fee_preview.volumetric_excess_percent
  source_type: metric_implementation
...
sql_pattern:
  ...
  sql_template: WITH actual_fees AS (SELECT sku_id, SUM(CASE WHEN mp_fee_type='ItemFees' AND transaction_type='Order' THEN charged_amount ELSE 0 END) AS actual_fee FROM zs_observe.amazon_disbursment
    ...
    fp JOIN actual_fees a ON fp.sku = a.sku_id WHERE fp.is_active = true AND fp.zen_status = true AND fp.is_duplicated = false;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.fee_preview.volumetric_gap
  description: Volumetric weight gap
  sql_template: SELECT sku, gross_weight, volumetric_weight, final_weight FROM zs_observe.amazon_fee_preview WHERE is_active = true AND zen_status = true AND is_duplicated = false AND volumetric_weight > gross_weight
    * 1.5;
```
```yaml
sql_pattern:
...
sql_pattern:
  ...
  sql_template: SELECT SUM(charged_amount_excluding_tax) AS taxable_revenue FROM zs_observe.amazon_oms WHERE is_active = true AND transaction_type = 'forward';
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.disbursement.principal_revenue
  description: Principal revenue
  sql_template: SELECT SUM(CASE WHEN mp_fee_type = 'ItemPrice' AND charged_amount_type = 'principal' THEN charged_amount ELSE 0 END) AS principal_revenue FROM zs_observe.amazon_disbursment
...
sql_pattern:
  ...
  sql_template: SELECT SUM(charged_amount) / NULLIF(COUNT(DISTINCT order_id), 0) AS average_order_value FROM zs_observe.amazon_oms WHERE is_active = true AND transaction_type
    = 'forward';
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.disbursement.shipping_cost_rate
  description: Marketplace shipping cost rate
  sql_template: SELECT sku_id, ABS(SUM(CASE WHEN mp_fee_type = 'ItemFees' AND charged_amount_type IN ('shipping fee','shipping chargeback') THEN charged_amount ELSE
    0 END)) / NULLIF(SUM(CASE WHEN mp_fee_type = 'ItemPrice' AND charged_amount_type = 'principal' THEN charged_amount ELSE 0 END), 0) AS shipping_cost_rate FROM
    zs_observe.amazon_disbursment WHERE is_active = true AND transaction_type = 'Order' GROUP BY sku_id;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.fee_preview.sku_fee_summary
  description: SKU fee burden
  sql_template: SELECT sku, gross_commission / NULLIF(charged_amount, 0) AS fee_burden FROM zs_observe.amazon_fee_preview WHERE is_active = true AND zen_status =
    true AND is_duplicated = false;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.disbursement.forward_fee
  description: Forward commission fee
  sql_template: SELECT sku_id, ABS(SUM(CASE WHEN mp_fee_type = 'ItemFees' AND charged_amount_type = 'commission' THEN charged_amount ELSE 0 END)) AS forward_fee FROM
    zs_observe.amazon_disbursment WHERE is_active = true AND transaction_type = 'Order' GROUP BY sku_id;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.settlement.net_settlement_per_order
  description: Net settlement per order
  sql_template: SELECT settlement_id, SUM(settled_amount) / NULLIF(COUNT(DISTINCT order_id), 0) AS net_settlement_per_order FROM zs_observe.amazon_settlement WHERE
    is_active = true GROUP BY settlement_id;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.disbursement.fee_recovery_on_returns
  description: Fee recovery on returns
  sql_template: SELECT charged_amount_type, (SUM(CASE WHEN transaction_type = 'Refund' THEN charged_amount ELSE 0 END) + SUM(CASE WHEN transaction_type = 'Fulfillment
    Fee Refund' THEN charged_amount ELSE 0 END)) / NULLIF(ABS(SUM(CASE WHEN transaction_type = 'Order' THEN charged_amount ELSE 0 END)), 0) AS recovery_rate FROM
    zs_observe.amazon_disbursment WHERE is_active = true AND mp_fee_type IN ('ItemFees','Item Fee Adjustment','ItemTCS','ItemTDS') GROUP BY charged_amount_type;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.oms.cancellation_rate
  description: Cancellation rate
  sql_template: SELECT COUNT(DISTINCT CASE WHEN transaction_type = 'cancel' THEN order_id END) / NULLIF(COUNT(DISTINCT CASE WHEN transaction_type IN ('forward','cancel')
    THEN order_id END), 0) AS cancellation_rate FROM zs_observe.amazon_oms WHERE is_active = true;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.oms.net_revenue_per_forward_order
  description: Net revenue per forward order
  sql_template: SELECT SUM(CASE WHEN transaction_type IN ('forward','reverse') THEN charged_amount ELSE 0 END) / NULLIF(COUNT(DISTINCT CASE WHEN transaction_type
    = 'forward' THEN order_id END), 0) AS net_revenue_per_forward_order FROM zs_observe.amazon_oms WHERE is_active = true;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.oms.return_value_rate
  description: Return value rate
  sql_template: SELECT AVG(ABS(CASE WHEN transaction_type = 'reverse' THEN charged_amount END)) / NULLIF(AVG(CASE WHEN transaction_type = 'forward' THEN charged_amount
    END), 0) AS return_value_rate FROM zs_observe.amazon_oms WHERE is_active = true;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.oms.discount_depth
  description: Discount depth
  sql_template: SELECT sku_id, SUM(item_promo_discount) / NULLIF(SUM(item_amount_excluding_tax), 0) AS discount_depth FROM zs_observe.amazon_oms WHERE is_active =
    true AND transaction_type = 'forward' GROUP BY sku_id;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.settlement.reimbursements_safet
  description: SAFE-T recovery rate proxy by settlement period
  sql_template: SELECT DATE_TRUNC('month', s.settlement_date) AS settlement_period, SUM(CASE WHEN s.type = 'SAFE-T Reimbursement' THEN s.total ELSE 0 END) / NULLIF(SUM(CASE WHEN r.return_reason IN ('DAMAGED_BY_FC','DAMAGED_BY_CARRIER','SWITCHEROO','UNAUTHORIZED_PURCHASE','NOT_AS_DESCRIBED')
    THEN ABS(r.charged_amount) ELSE 0 END), 0) AS safe_t_recovery_rate FROM zs_observe.amazon_settlement s JOIN zs_observe.amazon_returns r ON s.order_id = r.order_id
    WHERE s.is_active = true AND r.is_active = true GROUP BY 1;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.disbursement.return_loss
  description: Return loss by returned order
  sql_template: SELECT order_id, ABS(SUM(CASE WHEN transaction_type = 'Refund' AND mp_fee_type = 'ItemPrice' AND charged_amount_type = 'principal' THEN charged_amount
    ELSE 0 END)) + GREATEST(0, ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type = 'shipping fee' THEN charged_amount ELSE 0 END)) - SUM(CASE WHEN
    transaction_type = 'Fulfillment Fee Refund' AND charged_amount_type = 'shipping fee' THEN charged_amount ELSE 0 END)) + GREATEST(0, ABS(SUM(CASE WHEN transaction_type
    = 'Order' AND charged_amount_type = 'tech fee' THEN charged_amount ELSE 0 END)) - SUM(CASE WHEN transaction_type = 'Fulfillment Fee Refund' AND charged_amount_type
    = 'tech fee' THEN charged_amount ELSE 0 END)) + ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type = 'pickup fee' THEN charged_amount ELSE 0
    END)) + GREATEST(0, ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type = 'commission' THEN charged_amount ELSE 0 END)) - SUM(CASE WHEN transaction_type
    = 'Refund' AND charged_amount_type = 'refund commission' THEN charged_amount ELSE 0 END)) + GREATEST(0, ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type
    = 'closing fee' THEN charged_amount ELSE 0 END)) - SUM(CASE WHEN transaction_type = 'Refund' AND charged_amount_type = 'closing fee' THEN charged_amount ELSE
    0 END)) + 0.60 * ABS(SUM(CASE WHEN transaction_type = 'Order' AND charged_amount_type = 'tcs gst' THEN charged_amount ELSE 0 END)) AS return_loss FROM zs_observe.amazon_disbursment
    WHERE is_active = true AND transaction_type IN ('Order','Refund','Fulfillment Fee Refund') GROUP BY order_id;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.oms.revenue_by_state
  description: Revenue by destination state
  sql_template: SELECT destination_state, SUM(charged_amount) AS gross_revenue FROM zs_observe.amazon_oms WHERE is_active = true AND transaction_type = 'forward'
    GROUP BY destination_state ORDER BY gross_revenue DESC;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.disbursement.shipping_chargebacks
  description: Shipping chargebacks
  sql_template: SELECT order_id, sku_id, SUM(charged_amount) AS chargeback_amount FROM zs_observe.amazon_disbursment WHERE is_active = true AND charged_amount_type
    = 'shipping chargeback' GROUP BY order_id, sku_id ORDER BY ABS(SUM(charged_amount)) DESC;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.oms.b2b_b2c_split
  description: B2B vs B2C revenue split
  sql_template: SELECT CASE WHEN metadata IS NULL THEN 'International' ELSE metadata END AS segment, COUNT(DISTINCT order_id) AS orders, SUM(charged_amount) AS gross_revenue
    FROM zs_observe.amazon_oms WHERE is_active = true AND transaction_type = 'forward' GROUP BY 1 ORDER BY gross_revenue DESC;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.validation.oms_active_record_rate
  description: OMS active record rate validation
  sql_template: SELECT ROUND(100.0 * COUNT(*) FILTER (WHERE is_active = true) / NULLIF(COUNT(*), 0), 2) AS active_rate FROM zs_observe.amazon_oms;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.validation.oms_date_coverage
  description: OMS date coverage validation
  sql_template: SELECT MIN(created_date) AS earliest, MAX(created_date) AS latest, COUNT(DISTINCT DATE_TRUNC('month', created_date)) AS month_count FROM zs_observe.amazon_oms
    WHERE is_active = true AND transaction_type = 'forward';
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.validation.oms_settlement_order_count
  description: OMS-settlement order-count validation
  sql_template: SELECT 'OMS' AS source, COUNT(DISTINCT order_id) AS order_count, SUM(charged_amount) AS revenue FROM zs_observe.amazon_oms WHERE is_active = true
    AND transaction_type = 'forward' UNION ALL SELECT 'Settlement' AS source, COUNT(DISTINCT order_id), SUM(product_sales) FROM zs_observe.amazon_settlement WHERE
    is_active = true AND type = 'Order';
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.validation.fee_preview_coverage
  description: Fee preview coverage validation
  sql_template: SELECT COUNT(DISTINCT o.sku_id) AS oms_skus, COUNT(DISTINCT fp.sku) AS preview_skus, COUNT(DISTINCT CASE WHEN fp.sku IS NOT NULL THEN o.sku_id END)
    AS matched_skus FROM zs_observe.amazon_oms o LEFT JOIN zs_observe.amazon_fee_preview fp ON o.sku_id = fp.sku AND fp.is_active = true AND fp.zen_status = true
    AND fp.is_duplicated = false WHERE o.is_active = true AND o.transaction_type = 'forward';
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.validation.effective_tax_denominator
  description: Effective tax denominator validation
  sql_template: SELECT SUM(total_tax) / NULLIF(SUM(charged_amount_excluding_tax), 0) AS effective_tax_rate FROM zs_observe.amazon_oms WHERE is_active = true AND transaction_type
    = 'forward';
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.validation.settlement_total_formula
  description: Settlement total formula validation
  sql_template: SELECT settlement_id, SUM(total) AS recorded_total, SUM(product_sales + selling_fees + fba_fees + shipping_credits + promotional_rebates + gift_wrap_credits
    + other_transaction_fees + other - tcs_igst - tcs_cgst - tcs_sgst - tds) AS formula_total FROM zs_observe.amazon_settlement WHERE is_active = true GROUP BY settlement_id;
```
```yaml
sql_pattern:
  sql_ref: sql.amazon.validation.returns_source_active
  description: Returns source active-state validation
  sql_template: SELECT COUNT(*) AS active_return_rows FROM zs_observe.amazon_returns WHERE is_active = true;
```
...
review_item:
  ...
  required_resolution: No table card was created; kept only as a relationship mention where the DOCX provides no full schema.
  severity: medium
  evidence_refs: ev.amazon_oms.purpose.001
  status: resolved
  resolution_notes: No table card was created; kept only as a relationship mention where the DOCX provides no full schema.
```
```yaml
review_item:
  ...
  required_resolution: No table card was created; kept out of marketplace semantic cards until a full marketplace-owned schema is documented.
  severity: medium
  evidence_refs: ev.amazon_oms.purpose.001
  status: resolved
  resolution_notes: No table card was created; kept out of marketplace semantic cards until a full marketplace-owned schema is documented.
```
```yaml
review_item:
  ...
  required_resolution: No table card was created; treated as evidence mention only.
  severity: medium
  evidence_refs: ev.amazon.settlement_cycle.001; ev.amazon_settlement.purpose.001
  status: resolved
  resolution_notes: No table card was created; treated as evidence mention only.
```
```yaml
review_item:
  ...
  required_resolution: 'Converted to accepted availability constraint/rule: active analytics use OMS reverse rows for India; amazon_returns reason queries are prepared-for-reingest
    only.'
  severity: high
  evidence_refs: ev.amazon_returns.quality.001
  status: resolved
  resolution_notes: 'Converted to accepted availability constraint/rule: active analytics use OMS reverse rows for India; amazon_returns reason queries are prepared-for-reingest
    only.'
```
```yaml
review_item:
...
review_item:
  ...
  review_type: runtime_scope_binding_required
  related_card_type: column
  related_canonical_id: column.zs_observe.amazon_settlement.group_level_id
  issue: Canonical cards may include documented group_level_id scope specs and benchmark qualifiers, but the DOCX does not define runtime user/account scope-selection logic.
  required_resolution: Keep documented group_level_id values in canonical scope fields; provide selected runtime scope externally through a separate tenant/group/account-binding standard when generating user-specific SQL.
  severity: high
...
review_item:
  ...
  related_canonical_id: metric.marketplace.revenue_per_unit
  ...
  required_resolution: Amazon-specific OMS implementation remains removed; retain only the generic metric until an amazon_oms.quantity column card is documented.
  severity: medium
  evidence_refs: ev.amazon.kpi.revenue.001
  status: resolved
  resolution_notes: Amazon-specific OMS implementation remains removed; generic metric definition retained because the DOCX KPI table defines the business metric.
```
```yaml
review_item:
  ...
  required_resolution: Benchmarks were explicitly encoded as guidance fields/rules and are not validation failures.
  severity: low
  evidence_refs: ev.amazon.kpi.settlement.001; ev.amazon.return_analysis.001
  status: resolved
  resolution_notes: Benchmarks were explicitly encoded as guidance fields/rules and are not validation failures.
```
...
quality_gates:
  ...
  metric_implementation_patterns_present:
    status: pass
    notes: Amazon implementation cards include formulas, filters, grain, SQL refs, and metric patterns; pseudo formulas were converted to SQL-like formulas.
  reconciliation_scope:
    status: pass
    notes: Reconciliation profiles are marketplace-internal only.
  lazy_process_population:
    status: pass
    notes: Generic order_to_settlement lifecycle steps, weak process_variant cards, and remaining generic workflow-step descriptions were removed or replaced with
      DOCX-backed semantics.
  open_reviews:
    status: review_required
    notes: '2 review items remain open after correction: reconciliation tolerance and external runtime scope/account binding. Documented group_level_id specs are no longer blocked from canonical cards.'
  benchmark_scope_fields:
    status: pass
    notes: Structured metric benchmarks and documented group_level_id scope specs are present as guidance/context fields, not hard validation or account-binding cards.
  unified_edge_taxonomy_present:
    status: pass
    notes: Every candidate edge includes canonical_edge_type, inverse_edge_type, source/target types, materialize_inverse, canonical flag, and legacy aliases where
      applicable.
  ...
  sql_ref_coverage:
    status: pass
    notes: Every sql_ref referenced by a candidate card has a matching sql_pattern block.
```
...
candidate_coverage_summary:
  ...
  sql_patterns: 37
  open_reviews: 2
  cleanup_notes: Counts unchanged from V10 because V11 only corrected fields/manifest semantics; structured benchmarks and documented group_level_id scope specs were added without new cards or edges.
```
