# Shadowfax Logistics Knowledge — Parser Ready v6 Role-Split Manifest-Aligned Unified Edges
## 0. Document Metadata
```yaml
document_metadata:
  document_id: shadowfax_logistics_parser_ready_v6_role_split
  title: Shadowfax Logistics Knowledge — Parser Ready v6 Role-Split Manifest-Aligned Unified Edges
  domain: logistics
  vendor: Shadowfax
  ...
  scope: shadowfax_vendor_logistics_with_unified_edges
  logistics_only: true
  allowed_card_types:
  - metric
...
```yaml
source_evidence:
  id: evidence.shadowfax.vendor_scope
  source_document: Logistics KB Doc.docx
  summary: 'Vendor-specific extracted evidence for shadowfax: table role, coverage status, metrics, caveats, and joins.'
  confidence: high
```
...
```yaml
evidence_anchor_manifest:
  source_evidence_id: ev.shadowfax.coverage.reverse_only
  source_document: Logistics KB Doc.docx
  source_section: Courier Partner Summary and shiprocket_oms Courier Partner Distribution / Shadowfax reverse-only notes
  evidence_type: caveat
  supported_semantics:
  - Shadowfax is reverse-only/return QC in the available source
  - no dedicated native Shadowfax settlement table exists
  - indirect evidence appears through shiprocket_oms
  unsupported_semantics:
  - native Shadowfax freight or COD settlement metrics
  allowed_card_types:
  - platform
  - platform_context
  - metric_implementation
...
```yaml
review_item:
  review_id: review.shadowfax.no_native_table_reverse_only
  source_manifest: card_type_fit_manifest
  severity: high
  status: accepted_guardrail
  applies_to:
  - platform.shadowfax
  finding: Shadowfax is documented as reverse-only/indirect with no dedicated native settlement table.
  deterministic_action: Do not emit native Shadowfax table cards; only indirect/fallback metric implementations are allowed.
```
...
candidate_card:
  card_type: platform
  card_id: platform.shadowfax
  name: Shadowfax
  fields:
    name: Shadowfax
    description: Shadowfax logistics platform/vendor in the logistics domain.
    status: active
    confidence: curated
    source_documents:
    ...
    platform_name: Shadowfax
    platform_type: reverse_logistics
    active: true
    evidence_refs:
...
candidate_card:
  ...
  card_id: platform_context.shadowfax.in
  name: Shadowfax India
  fields:
    name: Shadowfax India
    description: India logistics context for Shadowfax.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
...
candidate_card:
  ...
  fields:
    ...
    implementations:
    - see metric_impl blocks in shadowfax_logistics_parser_ready_v6_role_split.md
    evidence_refs:
    - evidence.logistics_kb_doc
    - evidence.cognee_design_v4
    - evidence.unified_edge_scope
...
candidate_card:
  ...
  card_id: metric_impl.shiprocket_oms.shadowfax.return_shipment_count
  name: Shadowfax Return Shipment Count Implementation
  fields:
    name: Shadowfax Return Shipment Count Implementation
    description: Return Shipment Count implementation for Shadowfax using shiprocket_oms.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    implementation_name: Shadowfax Return Shipment Count
    metric_pattern: indirect_only_filtered_count_or_sum
    applicability: platform=shadowfax; table=zs_observe.shiprocket_oms; account filters via Account Data Binding only
    base_tables:
    - table.zs_observe.shiprocket_oms
    required_columns:
    - column.zs_observe.shiprocket_oms.courier_company
    semantic_filters:
    - courier_company contains Shadowfax
    formula_description: COUNT_IF(courier_company LIKE '%Shadowfax%')
    formula_sql: COUNT_IF(courier_company LIKE '%Shadowfax%')
    dependent_metrics: []
    allowed_dimensions:
    - courier_partner
    - date
    ...
    caveats: Shadowfax has no dedicated native table; reverse/QC evidence appears through Shiprocket OMS only.
    evidence_refs:
    - evidence.logistics_kb_doc
    - evidence.cognee_design_v4
    - evidence.unified_edge_scope
...
candidate_card:
  card_type: metric_implementation
  card_id: metric_impl.shiprocket_oms.shadowfax.declared_product_value
  name: Shadowfax Declared Product Value Implementation
  fields:
    name: Shadowfax Declared Product Value Implementation
    description: Declared Product Value implementation for Shadowfax using shiprocket_oms.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    implementation_name: Shadowfax Declared Product Value
    metric_pattern: indirect_only_filtered_count_or_sum
    applicability: platform=shadowfax; table=zs_observe.shiprocket_oms; account filters via Account Data Binding only
    base_tables:
    - table.zs_observe.shiprocket_oms
...
candidate_card:
  card_type: metric_implementation
  card_id: metric_impl.shiprocket_oms.shadowfax.shipment_count
  name: Shadowfax Shipment Count Implementation
  fields:
    name: Shadowfax Shipment Count Implementation
    description: Shipment Count implementation for Shadowfax using shiprocket_oms.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    implementation_name: Shadowfax Shipment Count
    metric_pattern: indirect_only_filtered_count_or_sum
    applicability: platform=shadowfax; table=zs_observe.shiprocket_oms; account filters via Account Data Binding only
    base_tables:
    - table.zs_observe.shiprocket_oms
...
candidate_card:
  card_type: metric_implementation
  card_id: metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table
  name: Shadowfax COD Remitted Amount Implementation
  fields:
    name: Shadowfax COD Remitted Amount Implementation
    description: COD Remitted Amount implementation for Shadowfax using shiprocket_oms.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    implementation_name: Shadowfax COD Remitted Amount
    ...
    unit: currency/count/percentage/days_by_metric
    precision: 2_decimal_places
    caveats: No dedicated Shadowfax settlement or invoice table exists in source KB.
    evidence_refs:
    - evidence.logistics_kb_doc
    - evidence.cognee_design_v4
    - evidence.unified_edge_scope
...
candidate_card:
  card_type: metric_implementation
  card_id: metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table
  name: Shadowfax Freight Billed Amount Implementation
  fields:
    name: Shadowfax Freight Billed Amount Implementation
    description: Freight Billed Amount implementation for Shadowfax using shiprocket_oms.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    implementation_name: Shadowfax Freight Billed Amount
    metric_pattern: unsupported_indirect_only
    applicability: platform=shadowfax; table=zs_observe.shiprocket_oms; account filters via Account Data Binding only
    base_tables:
    - table.zs_observe.shiprocket_oms
...
```yaml
candidate_edge:
  edge_id: edge.platform_context.shadowfax.in.belongs_to_platform.platform.shadowfax
  edge_type: BELONGS_TO_PLATFORM
  source: platform_context.shadowfax.in
  target: platform.shadowfax
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.platform.shadowfax.has_platform_context.platform_context.shadowfax.in
  edge_type: HAS_PLATFORM_CONTEXT
  source: platform.shadowfax
  target: platform_context.shadowfax.in
  fields:
    edge_family: platform_context
    ...
    materialized_from: edge.platform_context.shadowfax.in.belongs_to_platform.platform.shadowfax
```

```yaml
candidate_edge:
  edge_id: edge.metric.return_shipment_count.has_implementation.metric_impl.shiprocket_oms.shadowfax.return_shipment_count
  edge_type: HAS_IMPLEMENTATION
  source: metric.return_shipment_count
  target: metric_impl.shiprocket_oms.shadowfax.return_shipment_count
  fields:
    edge_family: metric_understanding
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.return_shipment_count.implements_metric.metric.return_shipment_count
  edge_type: IMPLEMENTS_METRIC
  source: metric_impl.shiprocket_oms.shadowfax.return_shipment_count
  ...
  fields:
    ...
    target_type: metric
    materialized_from: edge.metric.return_shipment_count.has_implementation.metric_impl.shiprocket_oms.shadowfax.return_shipment_count
```

```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.return_shipment_count.uses_table.table.zs_observe.shiprocket_oms
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.return_shipment_count.uses_column.column.zs_observe.shiprocket_oms.courier_company.28a8df7f
...
```yaml
candidate_edge:
  edge_id: edge.metric.declared_product_value.has_implementation.metric_impl.shiprocket_oms.shadowfax.declared_product_value
  edge_type: HAS_IMPLEMENTATION
  source: metric.declared_product_value
  target: metric_impl.shiprocket_oms.shadowfax.declared_product_value
  fields:
    edge_family: metric_understanding
    evidence_basis: metric_implementation.fields.metric_id
    confidence: high
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.declared_product_value.implements_metric.metric.declared_product_value
  edge_type: IMPLEMENTS_METRIC
  source: metric_impl.shiprocket_oms.shadowfax.declared_product_value
  ...
  fields:
    ...
    source_type: metric_implementation
    target_type: metric
    materialized_from: edge.metric.declared_product_value.has_implementation.metric_impl.shiprocket_oms.shadowfax.declared_product_value
```

```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.declared_product_value.uses_table.table.zs_observe.shiprocket_oms
  edge_type: USES_TABLE
  source: metric_impl.shiprocket_oms.shadowfax.declared_product_value
  target: table.zs_observe.shiprocket_oms
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.declared_product_value.uses_column.column.zs_observe.shiprocket_oms.courier_company.28a8df7f
  edge_type: USES_COLUMN
  source: metric_impl.shiprocket_oms.shadowfax.declared_product_value
  target: column.zs_observe.shiprocket_oms.courier_company
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.declared_product_value.uses_column.column.zs_observe.shiprocket_oms.charged_amount.8737448c
  edge_type: USES_COLUMN
  source: metric_impl.shiprocket_oms.shadowfax.declared_product_value
...
```yaml
candidate_edge:
  edge_id: edge.metric.shipment_count.has_implementation.metric_impl.shiprocket_oms.shadowfax.shipment_count
  edge_type: HAS_IMPLEMENTATION
  source: metric.shipment_count
  target: metric_impl.shiprocket_oms.shadowfax.shipment_count
  fields:
    edge_family: metric_understanding
    evidence_basis: metric_implementation.fields.metric_id
    confidence: high
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.shipment_count.implements_metric.metric.shipment_count
  edge_type: IMPLEMENTS_METRIC
  source: metric_impl.shiprocket_oms.shadowfax.shipment_count
  ...
  fields:
    ...
    source_type: metric_implementation
    target_type: metric
    materialized_from: edge.metric.shipment_count.has_implementation.metric_impl.shiprocket_oms.shadowfax.shipment_count
```

```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.shipment_count.uses_table.table.zs_observe.shiprocket_oms
  edge_type: USES_TABLE
  source: metric_impl.shiprocket_oms.shadowfax.shipment_count
  target: table.zs_observe.shiprocket_oms
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shiprocket_oms.shadowfax.shipment_count.uses_column.column.zs_observe.shiprocket_oms.courier_company.28a8df7f
  edge_type: USES_COLUMN
  source: metric_impl.shiprocket_oms.shadowfax.shipment_count
  target: column.zs_observe.shiprocket_oms.courier_company
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.metric.cod_remitted_amount.has_implementation.metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table
  edge_type: HAS_IMPLEMENTATION
  source: metric.cod_remitted_amount
  target: metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table
  fields:
    edge_family: metric_understanding
    evidence_basis: metric_implementation.fields.metric_id
    confidence: high
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table.implements_metric.metric.cod_remitted_amount
  edge_type: IMPLEMENTS_METRIC
  source: metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table
  ...
  fields:
    ...
    source_type: metric_implementation
    target_type: metric
    materialized_from: edge.metric.cod_remitted_amount.has_implementation.metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table
```

```yaml
candidate_edge:
  edge_id: edge.metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table.uses_table.table.zs_observe.shiprocket_oms
  edge_type: USES_TABLE
  source: metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table
  target: table.zs_observe.shiprocket_oms
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table.uses_column.column.zs_observe.shiprocket_oms.courier_company.28a8df7f
  edge_type: USES_COLUMN
  source: metric_impl.shadowfax_native.cod_remitted_amount.unsupported_no_table
  target: column.zs_observe.shiprocket_oms.courier_company
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.metric.freight_billed_amount.has_implementation.metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table
  edge_type: HAS_IMPLEMENTATION
  source: metric.freight_billed_amount
  target: metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table
  fields:
    edge_family: metric_understanding
    evidence_basis: metric_implementation.fields.metric_id
    confidence: high
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table.implements_metric.metric.freight_billed_amount
  edge_type: IMPLEMENTS_METRIC
  source: metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table
  ...
  fields:
    ...
    source_type: metric_implementation
    target_type: metric
    materialized_from: edge.metric.freight_billed_amount.has_implementation.metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table
```

```yaml
candidate_edge:
  edge_id: edge.metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table.uses_table.table.zs_observe.shiprocket_oms
  edge_type: USES_TABLE
  source: metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table
  target: table.zs_observe.shiprocket_oms
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table.uses_column.column.zs_observe.shiprocket_oms.courier_company.28a8df7f
  edge_type: USES_COLUMN
  source: metric_impl.shadowfax_native.freight_billed_amount.unsupported_no_table
  target: column.zs_observe.shiprocket_oms.courier_company
  fields:
...
```yaml
review_item:
  id: review.shadowfax_logistics_parser_ready_v6_role_split.business_flow_boundary
