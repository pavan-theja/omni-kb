# Logistics Domain Overview — Parser Ready v6 Role-Split Manifest-Aligned Unified Edges
## 0. Document Metadata
```yaml
document_metadata:
  document_id: logistics_domain_overview_parser_ready_v6_role_split
  title: Logistics Domain Overview — Parser Ready v6 Role-Split Manifest-Aligned Unified Edges
  domain: logistics
...
This index is readability-only. It does not create metric implementation cards in the domain overview; it points to the role/vendor docs that own table-specific implementations.

| metric_id | implementations | implementation docs |
|---|---:|---|
| `metric.actual_weight` | 1 | shiprocket_services_aggregator |
| `metric.average_delivery_attempts` | 1 | shiprocket_partner |
| `metric.average_freight_per_awb` | 2 | delhivery, shiprocket_services_aggregator |
| `metric.batch_settlement_amount` | 2 | ekart, shiprocket_services_aggregator |
| `metric.billable_weight` | 2 | delhivery, shiprocket_services_aggregator |
| `metric.cancelled_shipment_count` | 1 | shiprocket_partner |
| `metric.cod_collected_amount` | 6 | delhivery, dtdc, ekart, shiprocket_services_aggregator, xpressbees |
| `metric.cod_due_amount` | 1 | dtdc |
| `metric.cod_expected_amount` | 2 | delhivery, shiprocket_partner |
| `metric.cod_fee_amount` | 4 | delhivery, dtdc, ekart, shiprocket_services_aggregator |
| `metric.cod_gap_amount` | 1 | delhivery |
| `metric.cod_remittance_lag_days` | 4 | dtdc, ekart, shiprocket_partner, shiprocket_services_aggregator |
| `metric.cod_remitted_amount` | 11 | delhivery, dtdc, ecom_express, ekart, shadowfax, shiprocket_partner, shiprocket_services_aggregator, xpressbees |
| `metric.damaged_lost_shipment_count` | 1 | shiprocket_partner |
| `metric.declared_product_value` | 4 | delhivery, ekart, shadowfax, shiprocket_partner |
| `metric.delivered_shipment_count` | 3 | delhivery, shiprocket_partner, xpressbees |
| `metric.delivery_success_rate` | 1 | shiprocket_partner |
| `metric.delivery_tat_days` | 2 | dtdc, shiprocket_partner |
| `metric.dto_freight_amount` | 1 | delhivery |
| `metric.forward_freight_amount` | 4 | delhivery, dtdc, ekart, shiprocket_services_aggregator |
| `metric.forward_shipment_count` | 1 | shiprocket_partner |
| `metric.fov_insurance_amount` | 1 | delhivery |
| `metric.freight_billed_amount` | 8 | delhivery, dtdc, ecom_express, ekart, shadowfax, shiprocket_services_aggregator, xpressbees |
| `metric.freight_excluding_tax_amount` | 1 | shiprocket_services_aggregator |
| `metric.fuel_surcharge_amount` | 2 | delhivery, shiprocket_services_aggregator |
| `metric.gst_on_freight_amount` | 2 | delhivery, shiprocket_services_aggregator |
| `metric.in_transit_shipment_count` | 1 | shiprocket_partner |
| `metric.ndr_attempt_count` | 1 | shiprocket_partner |
| `metric.order_total_amount` | 1 | shiprocket_partner |
| `metric.payable_amount` | 1 | delhivery |
| `metric.peak_surcharge_amount` | 1 | delhivery |
| `metric.pickup_charge_amount` | 1 | delhivery |
| `metric.populated_native_record_count` | 1 | xpressbees |
| `metric.pos_settled_amount` | 1 | ekart |
| `metric.qr_cod_remitted_amount` | 1 | delhivery |
| `metric.return_shipment_count` | 2 | shadowfax, shiprocket_partner |
| `metric.rto_count` | 2 | delhivery, shiprocket_partner |
| `metric.rto_freight_amount` | 4 | delhivery, dtdc, ekart, shiprocket_services_aggregator |
| `metric.rto_rate` | 1 | shiprocket_partner |
| `metric.rto_tat_days` | 1 | shiprocket_partner |
| `metric.settlement_batch_count` | 3 | delhivery, dtdc, ekart |
| `metric.shipment_count` | 4 | delhivery, ecom_express, shadowfax, shiprocket_partner |
| `metric.shipment_pickup_lag_days` | 1 | shiprocket_partner |
| `metric.sparse_native_record_count` | 1 | xpressbees |
| `metric.unique_awb_count` | 6 | delhivery, dtdc, ekart, shiprocket_partner, shiprocket_services_aggregator, xpressbees |
...
candidate_card:
  ...
  fields:
    description: Reusable lifecycle where a channel or marketplace order becomes a logistics shipment with an AWB and courier assignment.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    operational_test: A downstream system should be able to decide whether an object moved from order_confirmed to awb_assigned using evidence cards and relationships, not prose alone.
    applicability_scope:
...
candidate_card:
  ...
  fields:
    ...
    reconciliation_profiles: []
    operational_test: A downstream system should be able to decide whether an object moved from shipment_created to courier_handoff_confirmed using evidence cards and relationships, not prose alone.
    applicability_scope:
      scope_level: platform_type
      platform_types: *id001
      notes:
...
candidate_card:
  ...
  fields:
    ...
    operational_test: A downstream system should be able to decide whether an object moved from awb_assigned to freight_invoice_available using evidence cards and relationships, not prose alone.
    applicability_scope:
      scope_level: platform_type
      platform_types: *id001
      notes:
...
candidate_card:
  ...
  fields:
    description: Reusable process for comparing actual freight billed against expected freight based on components, zone, weight, COD, RTO, DTO, and taxes.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    reconciliation_profiles:
    - reconciliation_profile.freight_charge_validation
    operational_test: A downstream system should be able to decide whether an object moved from expected_freight_basis_identified to freight_variance_classified using evidence cards and relationships, not prose alone.
    applicability_scope:
      scope_level: platform_type
      platform_types: *id001
      notes:
...
candidate_card:
  ...
  fields:
    description: Reusable lifecycle where delivered COD shipments are expected to produce courier or aggregator remittance evidence.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    operational_test: A downstream system should be able to decide whether an object moved from delivered_cod_shipment to cod_remittance_recorded using evidence cards and relationships, not prose alone.
    applicability_scope:
      scope_level: platform_type
      platform_types: *id001
      notes:
...
candidate_card:
  ...
  card_id: process_variant.shiprocket_logistics_partner.operational_handoff
  name: Shiprocket Logistics Partner Operational Handoff
  fields:
    name: Shiprocket Logistics Partner Operational Handoff
    description: Shiprocket operationally receives Non-FBF shipment handoff, assigns AWB, routes to an underlying courier, and exposes operational delivery/RTO/return state through shiprocket_oms.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    - logistics_gold_std_raw_md_frame_v3.md
    - logistics_gold_std_canonical_card_frame_v5.md
    - marketplace_cleanup_manifest_consolidated_v2.md
    created_by: kb_authoring_assistant
    updated_by: kb_authoring_assistant
    version: 6.0-role-split
    variant_name: Shiprocket Logistics Partner Operational Handoff
    base_process_ids:
    - business_process.order_to_shipment_flow
    - business_process.warehouse_to_courier_handoff
    - business_process.shipment_tracking_and_delivery
    ...
    platform_roles:
    - logistics_partner
    - operational_handoff_partner
    - awb_assignment_source
    - underlying_courier_router
    - shipment_status_source
    operational_evidence_tables:
    - table.zs_observe.shiprocket_oms
    financial_evidence_tables:
    - table.zs_observe.shiprocket_invoice
    - table.zs_observe.shiprocket_settlement
    underlying_courier_dimension_columns:
    - column.zs_observe.shiprocket_oms.courier_company
    - column.zs_observe.shiprocket_oms.master_courier
    - column.zs_observe.shiprocket_oms.fulfilment_channel
    affected_metrics:
    - metric.shipment_count
    - metric.unique_awb_count
    - metric.delivered_shipment_count
    ...
    - metric.shipment_pickup_lag_days
    - metric.delivery_tat_days
    affected_reconciliation_profiles:
    - reconciliation_profile.order_to_shipment
    - reconciliation_profile.shipment_to_freight_invoice
    - reconciliation_profile.cod_expected_to_courier_remittance
    boundary_note: This variant models reusable Shiprocket operational logistics partner behavior only. Tenant/group-specific Shopify→Shiprocket usage belongs in Business Flow Binding.
    evidence_refs:
    - evidence.logistics_kb_doc
    - evidence.cognee_design_v4
    - evidence.unified_edge_scope
...
candidate_card:
  card_type: process_variant
  card_id: process_variant.shiprocket_services_aggregator.financial_intermediary
  name: Shiprocket Services Aggregator Financial Intermediary
  fields:
    name: Shiprocket Services Aggregator Financial Intermediary
    description: Shiprocket consolidates freight billing and COD remittance across underlying courier partners even when the physical courier is Delhivery, DTDC, XpressBees, Ekart, Shadowfax, or Ecom Express.
    status: active
    confidence: curated
    source_documents:
    - Logistics KB Doc.docx
    ...
    updated_by: kb_authoring_assistant
    version: 6.0-role-split
    variant_name: Shiprocket Services Aggregator Financial Intermediary
    base_process_ids:
    - business_process.shipment_to_freight_invoice
    - business_process.freight_charge_validation
    - business_process.cod_delivery_to_courier_remittance
    ...
    platform_roles:
    - courier_aggregator
    - freight_billing_intermediary
    - cod_remittance_intermediary
    - underlying_courier_financial_router
    - fallback_evidence_source_for_indirect_couriers
    operational_evidence_tables:
    - table.zs_observe.shiprocket_oms
    financial_evidence_tables:
    - table.zs_observe.shiprocket_invoice
    ...
    - reconciliation_profile.freight_charge_validation
    - reconciliation_profile.cod_expected_to_courier_remittance
    - reconciliation_profile.low_confidence_native_to_fallback
    boundary_note: This variant models reusable Shiprocket financial aggregation behavior only. Tenant/group bank destination and account participation belong in Business Flow Binding.
    evidence_refs:
    - evidence.logistics_kb_doc
    - evidence.cognee_design_v4
    - evidence.unified_edge_scope
...
```yaml
candidate_edge:
  edge_id: edge.business_process.order_to_shipment_flow.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
  edge_type: HAS_PROCESS_VARIANT
  source: business_process.order_to_shipment_flow
  target: process_variant.shiprocket_logistics_partner.operational_handoff
  fields:
    edge_family: process_understanding
...
```yaml
candidate_edge:
  edge_id: edge.process_variant.shiprocket_logistics_partner.operational_handoff.extends_process.business_process.order_to_shipment_flow
  edge_type: EXTENDS_PROCESS
  source: process_variant.shiprocket_logistics_partner.operational_handoff
  target: business_process.order_to_shipment_flow
  fields:
    edge_family: process_understanding
    ...
    target_type: business_process
    materialized_from: edge.business_process.order_to_shipment_flow.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
```
```yaml
candidate_edge:
  edge_id: edge.business_process.warehouse_to_courier_handoff.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
  edge_type: HAS_PROCESS_VARIANT
  source: business_process.warehouse_to_courier_handoff
  target: process_variant.shiprocket_logistics_partner.operational_handoff
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.process_variant.shiprocket_logistics_partner.operational_handoff.extends_process.business_process.warehouse_to_courier_handoff
  edge_type: EXTENDS_PROCESS
  source: process_variant.shiprocket_logistics_partner.operational_handoff
  target: business_process.warehouse_to_courier_handoff
  fields:
    ...
    source_type: process_variant
    target_type: business_process
    materialized_from: edge.business_process.warehouse_to_courier_handoff.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
```
```yaml
candidate_edge:
  edge_id: edge.business_process.shipment_tracking_and_delivery.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
  edge_type: HAS_PROCESS_VARIANT
  source: business_process.shipment_tracking_and_delivery
  target: process_variant.shiprocket_logistics_partner.operational_handoff
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.process_variant.shiprocket_logistics_partner.operational_handoff.extends_process.business_process.shipment_tracking_and_delivery
  edge_type: EXTENDS_PROCESS
  source: process_variant.shiprocket_logistics_partner.operational_handoff
  target: business_process.shipment_tracking_and_delivery
  fields:
    ...
    source_type: process_variant
    target_type: business_process
    materialized_from: edge.business_process.shipment_tracking_and_delivery.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
```
```yaml
candidate_edge:
  edge_id: edge.business_process.delivery_rto_return_resolution.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
  edge_type: HAS_PROCESS_VARIANT
  source: business_process.delivery_rto_return_resolution
  target: process_variant.shiprocket_logistics_partner.operational_handoff
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.process_variant.shiprocket_logistics_partner.operational_handoff.extends_process.business_process.delivery_rto_return_resolution
  edge_type: EXTENDS_PROCESS
  source: process_variant.shiprocket_logistics_partner.operational_handoff
  target: business_process.delivery_rto_return_resolution
  fields:
    ...
    source_type: process_variant
    target_type: business_process
    materialized_from: edge.business_process.delivery_rto_return_resolution.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
```
```yaml
candidate_edge:
  edge_id: edge.business_process.cod_delivery_to_courier_remittance.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
  edge_type: HAS_PROCESS_VARIANT
  source: business_process.cod_delivery_to_courier_remittance
  target: process_variant.shiprocket_logistics_partner.operational_handoff
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.process_variant.shiprocket_logistics_partner.operational_handoff.extends_process.business_process.cod_delivery_to_courier_remittance
  edge_type: EXTENDS_PROCESS
  source: process_variant.shiprocket_logistics_partner.operational_handoff
  target: business_process.cod_delivery_to_courier_remittance
  fields:
    ...
    source_type: process_variant
    target_type: business_process
    materialized_from: edge.business_process.cod_delivery_to_courier_remittance.has_process_variant.process_variant.shiprocket_logistics_partner.operational_handoff
```
```yaml
candidate_edge:
  edge_id: edge.business_process.shipment_to_freight_invoice.has_process_variant.process_variant.shiprocket_services_aggregator.financial_intermediary
  edge_type: HAS_PROCESS_VARIANT
  source: business_process.shipment_to_freight_invoice
  target: process_variant.shiprocket_services_aggregator.financial_intermediary
  fields:
    edge_family: process_understanding
    evidence_basis: business_process.fields.process_variants
    confidence: high
...
```yaml
candidate_edge:
  edge_id: edge.process_variant.shiprocket_services_aggregator.financial_intermediary.extends_process.business_process.shipment_to_freight_invoice
  edge_type: EXTENDS_PROCESS
  source: process_variant.shiprocket_services_aggregator.financial_intermediary
  target: business_process.shipment_to_freight_invoice
  fields:
    edge_family: process_understanding
    evidence_basis: materialized inverse of HAS_PROCESS_VARIANT
    ...
    source_type: process_variant
    target_type: business_process
    materialized_from: edge.business_process.shipment_to_freight_invoice.has_process_variant.process_variant.shiprocket_services_aggregator.financial_intermediary
```
```yaml
candidate_edge:
  edge_id: edge.business_process.freight_charge_validation.has_process_variant.process_variant.shiprocket_services_aggregator.financial_intermediary
  edge_type: HAS_PROCESS_VARIANT
  source: business_process.freight_charge_validation
  target: process_variant.shiprocket_services_aggregator.financial_intermediary
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.process_variant.shiprocket_services_aggregator.financial_intermediary.extends_process.business_process.freight_charge_validation
  edge_type: EXTENDS_PROCESS
  source: process_variant.shiprocket_services_aggregator.financial_intermediary
  target: business_process.freight_charge_validation
  fields:
    ...
    source_type: process_variant
    target_type: business_process
    materialized_from: edge.business_process.freight_charge_validation.has_process_variant.process_variant.shiprocket_services_aggregator.financial_intermediary
```
```yaml
candidate_edge:
  edge_id: edge.business_process.cod_delivery_to_courier_remittance.has_process_variant.process_variant.shiprocket_services_aggregator.financial_intermediary
  edge_type: HAS_PROCESS_VARIANT
  source: business_process.cod_delivery_to_courier_remittance
  target: process_variant.shiprocket_services_aggregator.financial_intermediary
  fields:
...
```yaml
candidate_edge:
  edge_id: edge.process_variant.shiprocket_services_aggregator.financial_intermediary.extends_process.business_process.cod_delivery_to_courier_remittance
  edge_type: EXTENDS_PROCESS
  source: process_variant.shiprocket_services_aggregator.financial_intermediary
  target: business_process.cod_delivery_to_courier_remittance
  fields:
    ...
    source_type: process_variant
    target_type: business_process
    materialized_from: edge.business_process.cod_delivery_to_courier_remittance.has_process_variant.process_variant.shiprocket_services_aggregator.financial_intermediary
```
...
```yaml
review_item:
  id: review.logistics_domain_overview_parser_ready_v6_role_split.business_flow_boundary
