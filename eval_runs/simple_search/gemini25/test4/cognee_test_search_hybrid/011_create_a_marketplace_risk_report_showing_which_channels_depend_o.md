...
- started_at: `2026-05-27T06:42:17+00:00`
- duration_seconds: `81.081`
- RAG_COMPLETION_seconds: `50.172`
- GRAPH_COMPLETION_seconds: `30.909`
...

{
  ...
  "metric_logic": {
    ...
    "denominator": null,
    "formula": "A distinct list of OMS systems and the marketplaces/channels that depend on them for Mensa Brands.",
    "numerator": null
  },
  "missing_or_ambiguous": "The exact presence and naming of an `is_active` column in `zs_observe.amazon_oms`, `zs_observe.increff_sales`, and `zs_observe.shopify_oms` are inferred based on common data modeling practices. For `zs_observe.myntra_oms`, the `is_active = true` filter is explicitly excluded as per `rule.myntra.oms_no_is_active`. The presence of a `sales_channel` column in `zs_observe.increff_sales` is inferred based on its role as an OMS/WMS for cross-channel operations. The exact string values for directly mapped marketplaces (e.g., 'Amazon', 'Myntra', 'Shopify') are inferred from the platform context. The report identifies which channels depend on which OMS systems, which can form the basis for a 'risk report' but does not quantify 'risk' itself.",
  ...
  "require_tables": [
    ...
    },
    {
      ...
      "reason": "`query_pattern.myntra.gross_sales_oms` uses this table and `rule.myntra.oms_no_is_active` explicitly mentions this table. Myntra is a marketplace.",
      "role": "Primary Source for Myntra OMS data",
      "selected?": "Yes"
    },
    {
      "field": "zs_observe.unicommerce",
      "reason": "Identified as a central OMS system for Mensa Brands, containing `sales_channel` to list connected marketplaces.",
  ...
  "required_fields": [
    ...
    {
      "field": "is_active",
      "reason": "Inferred as a mandatory filter (`is_active = true`) for data validity and consistency across operational tables, where applicable. For Myntra OMS, it's explicitly excluded by `rule.myntra.oms_no_is_active`.",
