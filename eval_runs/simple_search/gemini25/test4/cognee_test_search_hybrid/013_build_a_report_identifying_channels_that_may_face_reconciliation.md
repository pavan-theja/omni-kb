...
- started_at: `2026-05-27T06:44:36+00:00`
- duration_seconds: `206.364`
- RAG_COMPLETION_seconds: `127.588`
- GRAPH_COMPLETION_seconds: `78.775`
...
{
  "filters": [],
  "joins": "No direct joins on physical tables are performed for this metadata report. Information is aggregated from various query pattern and rule definitions.",
  "metric_logic": {
    "aggregation_grain": "channel_name, potential_delay_reason",
    "deduplication_rule": "Each channel is listed once with all relevant reasons for potential delays.",
    "denominator": null,
    "formula": "A qualitative report identifying channels with contextual evidence of potential reconciliation delays due to marketplace-based settlements. There is no direct quantitative metric for 'delay' in the provided context, rather indicators of complexity or issues.",
    "numerator": null
  },
  "missing_or_ambiguous": "Quantifying the actual 'delay' or severity is not possible from the given context, only identifying channels that have known complexities or explicit mentions of reconciliation issues or timing gaps. The `group_level_id` for Mensa Brands is generally '22', but specific marketplaces like JioMart (`group_level_id = 26`) may have their own. For this metadata report, we infer the Mensa Brands scope to cover these marketplaces.",
  ...
  "rejected_sources": [
    ...
    "query_pattern.jiomart.marketplace.6_1_total_gmv",
    "query_pattern.jiomart.marketplace.6_2_return_rate",
    "query_pattern.jiomart.marketplace.6_3_average_order_value_aov",
    "query_pattern.jiomart.marketplace.6_4_net_settled_revenue",
    "query_pattern.jiomart.marketplace.6_5_seller_coupon_discount_impact",
    "query_pattern.jiomart.marketplace.6_6_monthly_forward_orders_and_gmv",
    "query_pattern.jiomart.marketplace.9_mandatory_query_filters",
    "query_pattern.jiomart.oms.7_1_monthly_gmv_clean_rows_only",
    "query_pattern.jiomart.oms.7_2_clean_forward_orders_exclude_column_shifted_rows",
    "query_pattern.jiomart.oms.7_3_intra_state_vs_inter_state_gst_analysis",
    "query_pattern.jiomart.oms.7_4_sku_level_performance",
    "query_pattern.jiomart.oms.7_5_oms_settlement_join",
    "query_pattern.jiomart.oms.7_6_return_rate",
    "query_pattern.jiomart.returns.7_1_return_volume_by_type",
    "query_pattern.jiomart.returns.7_2_return_cycle_time_rto_turnaround",
    "query_pattern.jiomart.returns.7_3_returns_oms_reconciliation",
    "query_pattern.jiomart.returns.7_4_sku_level_return_rate",
    "query_pattern.jiomart.settlement.8_1_net_settlement_per_order_structured_rows",
    "query_pattern.jiomart.settlement.8_2_monthly_settlement_summary",
    "query_pattern.jiomart.shipment.7_1_shipment_status_summary",
    "query_pattern.jiomart.shipment.7_2_monthly_gmv",
    "query_pattern.jiomart.shipment.7_3_top_skus",
    "query_pattern.jiomart.settlement.8_5_credit_debit_notes",
    "rule.jiomart.cast_return_amounts",
    "rule.jiomart.event_not_null_for_structured_analysis",
    ...
    "rule.jiomart.use_created_date_for_oms_time_series",
    "rule.jiomart.use_sku_id_for_oms_settlement",
    "rule.jiomart.use_sku_id_mp_sin_combined_key",
    "validation_test.jiomart.no_missing_active_filter",
    "validation_test.jiomart.no_shipment_cross_join",
    "validation_test.jiomart.order_id_join_preferred",
    "validation_test.jiomart.returns_amount_cast",
    "validation_test.jiomart.settlement_preaggregation",
    "validation_test.jiomart.tcs_rate_anomaly",
  ...
  "require_tables": [
    {
      ...
      "reason": "Used in reconciliation patterns for Meesho, such as sales-settlement matching.",
      "role": "Supporting Table for Meesho Reconciliation",
      "selected?": "Yes"
    },
    {
      "field": "zs_observe.meesho_settlement",
      "reason": "Primary table for Meesho settlements, explicitly referenced in many reconciliation patterns.",
      "role": "Supporting Table for Meesho Reconciliation",
      "selected?": "Yes"
    },
    {
      "field": "zs_observe.meesho_reverse",
      "reason": "Involved in reverse logistics and return settlement reconciliation for Meesho.",
      "role": "Supporting Table for Meesho Reconciliation",
      "selected?": "Yes"
    },
    {
      "field": "zs_observe.jiomart_oms",
      "reason": "Implied source for OMS data in JioMart's 3-way reconciliation and unreconciled order patterns.",
      "role": "Supporting Table for JioMart Reconciliation",
      "selected?": "Yes"
    },
    {
      "field": "zs_observe.jiomart_settlement",
      "reason": "Primary table for JioMart settlements, used in OMS-settlement matching and other reconciliation patterns.",
      "role": "Supporting Table for JioMart Reconciliation",
      "selected?": "Yes"
    },
    {
      "field": "zs_observe.jiomart_returns",
      "reason": "Involved in returns-OMS-settlement reconciliation for JioMart.",
      "role": "Supporting Table for JioMart Reconciliation",
      "selected?": "Yes"
    }
  ],
  "required_fields": [
    {
      "field": "channel_name",
      "reason": "Literal string to identify the marketplace/channel.",
      "role": "Dimension (Marketplace Identifier)",
      "selected?": "Yes"
    },
    {
      "field": "potential_delay_reason",
      "reason": "Literal string describing the specific contextual evidence for potential reconciliation delays.",
      "role": "Dimension (Reconciliation Delay Reason)",
      "selected?": "Yes"
    }
  ],
  "selected_source": null,
  "sql_skeleton": "SELECT 'Meesho' AS channel_name, 'Explicit timing gaps in reverse logistics reconciliation (rule.meesho.reverse_logistics_timing_gap)' AS potential_delay_reason\nUNION ALL\nSELECT 'Meesho' AS channel_name, 'Numerous reconciliation patterns for sales, returns, expenses, and TCS indicating complexity' AS potential_delay_reason\nUNION ALL\nSELECT 'JioMart' AS channel_name, 'Presence of unreconciled OMS orders (query_pattern.jiomart.marketplace.7_3_unreconciled_oms_orders_not_in_settlement)' AS potential_delay_reason\nUNION ALL\nSELECT 'JioMart' AS channel_name, 'Complex aggregation requirements for settlement before join (rule.jiomart.aggregate_settlement_before_join) and specific OMS-settlement join keys (rule.jiomart.use_order_id_for_oms_settlement)' AS potential_delay_reason;"
}
...

{
  ...
  "missing_or_ambiguous": "Quantifying the actual 'delay' or severity is not possible from the given context, only identifying channels that have known complexities or explicit mentions of reconciliation issues or timing gaps. The `group_level_id` for Mensa Brands is generally '22'. For this metadata report, we infer that these marketplaces are relevant to Mensa Brands given the context, even if the specific `account_data_binding` examples for Meesho/JioMart show different `group_level_id` values (e.g., '24' or '26') for other tenants. The report is based on the general characteristics of these marketplace's settlement processes.",
  "rejected_or_ambiguous_fields": [],
  "rejected_sources": [
    "tcs_tds_summary",
    "metric_impl.meesho.total_cost_of_returns",
